...
bool MySql::initsql() { //初始化并建立一个数据库连接
	//连接数据库
    QSqlDatabase db = QSqlDatabase::addDatabase("QODBC");//创建一个QSqlDatabase对象db,使用QODBC驱动来连接数据库
...
//基于MySQL数据库的登录函数 Name和Password表示用户名和密码
...
bool MySql::Regist(QString Name, QString Password, QString Mail) {
    ...
    query->last();//将查询结果的游标移动到最后一条记录.这是为了获取结果集中的总记录数
    int record = query->at() + 1;//获取当前记录的索引位置.由于last()方法将游标移动到最后一条记录,所以当前记录的索引位置就是结果集的总记录数减一

    //如果存在记录，则返回false表示注册失败
